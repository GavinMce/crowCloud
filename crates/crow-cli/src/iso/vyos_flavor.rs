...
/// post-install config step entirely for redeploys/replacements of a
/// box whose hardware (and therefore PCI addressing) is already known.
///
...
pub struct VyosFlavorConfig {
    /// Everything static about the fabric config -- VLANs, IPs, ASN,
    /// NAT, DNS forwarders, etc. `trunk_interface`/`uplink_interface`
    /// are ignored by this module; the generated script resolves the
    /// real interface names at boot from `trunk_pci`/`uplink_pci`
    /// instead, since kernel-assigned `ethN` names aren't stable across
    /// a NIC swap (confirmed live: this fabric's own eth0/eth1/eth2
    /// naming already depends on driver probe order).
    pub base: VyosBuildConfig,
    /// PCI bus address of the fabric trunk NIC (e.g. `0000:01:00.0`),
    /// stable across a card swap in the same physical slot -- read via
    /// `readlink -f /sys/class/net/<iface>/device` on the target box.
    pub trunk_pci: String,
    pub uplink_pci: String,
}

const FIND_IFACE_BY_PCI: &str = r#"find_iface_by_pci() {
    local target_pci="$1"
    local dev iface pci
...
/// `render_cron_entry`). Resolves the trunk/uplink PCI addresses to
/// whatever their current kernel interface names are, then applies the
/// exact same `set` commands `iso vyos apply` would push over SSH --
/// reuses `render_configure_script` directly rather than re-deriving
/// the fabric config in bash, so the two mechanisms can't drift apart.
...
TRUNK_IF="$(find_iface_by_pci "${{TRUNK_PCI}}")" || {{
    echo "No interface found at PCI address ${{TRUNK_PCI}} (trunk) -- refusing to apply a partial fabric config" >&2
    exit 1
}}
UPLINK_IF="$(find_iface_by_pci "${{UPLINK_PCI}}")" || {{
    echo "No interface found at PCI address ${{UPLINK_PCI}} (uplink) -- refusing to apply a partial fabric config" >&2
    exit 1
}}

echo "Resolved trunk=${{TRUNK_IF}} (${{TRUNK_PCI}}), uplink=${{UPLINK_IF}} (${{UPLINK_PCI}})"
...
VBASH
"#,
        trunk_pci = cfg.trunk_pci,
        uplink_pci = cfg.uplink_pci,
        find_iface_by_pci = FIND_IFACE_BY_PCI,
...
mod tests {
    ...
    fn cfg() -> VyosFlavorConfig {
        VyosFlavorConfig {
            ...
            trunk_pci: "0000:01:00.0".into(),
            uplink_pci: "0000:02:00.0".into(),
        }
    }

    #[test]
    fn resolves_interfaces_by_pci_address_not_kernel_name() {
        let out = render_fabric_init_script(&cfg());
        assert!(out.contains(r#"TRUNK_PCI="0000:01:00.0""#));
        assert!(out.contains(r#"UPLINK_PCI="0000:02:00.0""#));
        assert!(out.contains("find_iface_by_pci"));
        // The config's own trunk_interface/uplink_interface (kernel
        // names, not stable across a card swap) must never leak in --
        // only the dynamically-resolved shell variables should.
    ...
    #[test]
    fn refuses_to_apply_a_partial_config_if_either_interface_is_missing() {
        let out = render_fabric_init_script(&cfg());
